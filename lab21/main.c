...
int main(int argc, char* argv[])
{
    ...
    // Initialize the whole vector and print the first and last element on processor 0
    double v_first;
    double v_last;
    vector v_all = new_vector(N); 

    // if( my_rank == 0)
    // { 
    //     for(int i=1; i <= N ; i++)
    //     { 
    //         vget(v_all, i) = sqrt((double)(i)); 
    //     }
    //     v_first = vget(v_all, 1); 
    //     v_last = vget(v_all, N);
    ...
    if( my_rank != 0)
    {
        ...
        MPI_Send(v_local.val, N_local, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
    }
    else
    {
        ...
        {
            vget(v_all, j) = vget(v_local, j);
        }
        for(int i=1; i<comm_sz; i++)
        {
            MPI_Recv(&norm_squared_local, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
            norm_squared += norm_squared_local ;
            MPI_Recv(v_temp, N_local, MPI_DOUBLE, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
            for (int j = 1; j <= N_local; j++)
            {
                vget(v_all, i*N_local+j) = v_temp[j-1];
            }
        }
        v_first = vget(v_all, 1); 
        v_last = vget(v_all, N);
    ...
    // Send back the normalized local vectors to Processor 0
    if( my_rank != 0)
    {
        MPI_Send(v_local.val, N_local, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
    }
    ...
    // // Normalize the whole vector with the global norm
    // if( my_rank == 0 )
    // {
    //     for(int i=1; i <= N ; i++)
    //     { vget(v_all,i) = vget(v_all, i)/sqrt(norm_squared); }
