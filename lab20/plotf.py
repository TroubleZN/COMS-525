def plotf():
    ...
    thread_id = np.zeros(NumIntervals, dtype=int)
    ...
    for k in range(NumIntervals):
        plt.plot([a[k],b[k]],[-float(k),-float(k)], color=col[thread_id[k]-1], linewidth =2)
    plt.title("Subintervals used in adaptive quadrature")
    plt.axis("off")
    plt.savefig('adapt_quad_ser_1.png', dpi =400 , bbox_inches ='tight')
    ...
    plt.savefig('adapt_quad_ser_2.png', dpi =400 , bbox_inches ='tight')
    plt.show()
