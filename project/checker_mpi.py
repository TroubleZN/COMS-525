...
def alpha_beta_search(board, agent_color, maxdepth=float('inf'), heuristic_type=0):
    ...
    best_result = None
    moves_all, results_all = all_moves_color(board, color)
    n_moves = len(moves_all)
    n_moves_local = n_moves // n_workers + 1

    score_local = []
    results_temp = results_all[rank*n_moves_local:min((rank+1)*n_moves_local, n_moves)]
    for i, result in enumerate(results_temp):
        v = min_value(result, another_color(color), -float('inf'), beta, 1, maxdepth)
        score_local.append(v)

    if rank == 0:
        for i in range(1, n_workers):
            score_remote = comm.recv(source=i, tag=11)
            score_local.extend(score_remote)
        index = score_local.index(max(score_local))
        best_action = moves_all[index]
        best_result = copy.deepcopy(results_all[index])
    else:
        comm.send(score_local, dest=0, tag=11)
...
def main_user():
    if rank == 0:
        ...
        for i in range(n_workers-1):
            comm.send(user_color, dest=i+1, tag=1)
    else:
        user_color = comm.recv(source=0, tag=1)
    ...
    while not is_over(board, color) and steps <= 100:
        if user_color == color:
            if rank == 0:
                ...
                for i, pos in enumerate(move2):
                    if pos[0] == piece_position and pos[1] == target_position:
                        board = results_all[i]
                        move = moves_all[i]
                ...
                for i in range(n_workers - 1):
                    comm.send(board, dest=i+1, tag=2)
            else:
            ...
            best_action, best_result = alpha_beta_search(board, color, 6)

            if rank == 0:
                board = best_result
                print("The move made is " + " -> ".join(position_trans(pos) for pos in best_action))
                print("Time used: ", round(time.time()-start, 3), "sed")
                board.display()
                for i in range(n_workers - 1):
                    comm.send(board, dest=i+1, tag=3)
            else:
                board = comm.recv(source=0, tag=3)
